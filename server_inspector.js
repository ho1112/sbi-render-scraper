...
module.exports.scrapeDividend = async function scrapeDividend(options = {}) {
  ...
  try {
    ...
    if (isLocal && puppeteerLocal && !forceHeadless) {
      ...
      browser = await puppeteerLocal.launch({
        ...
        args: ['--start-maximized'] // 최대화된 상태로 시작
           ...
           } else if (isLocal && forceHeadless) {
         // 로컬 환경에서 headless 모드 - puppeteer-extra 사용 (stealth 모드)
         console.log('로컬 환경에서 headless 모드로 실행합니다 (stealth 모드)...');
         browser = await puppeteerExtra.launch({
           headless: true,
           slowMo: 1000,  // 1초 슬로우 모드
           args: [
             '--no-sandbox',
         ...
         // Render 환경에서 headless 모드 - puppeteer-extra 사용 (stealth 모드)
         console.log('Render 환경에서 headless 모드로 실행합니다 (stealth 모드)...');
         browser = await puppeteerExtra.launch({
           args: [
             ...chromium.args,
             '--disable-web-security',
    ...
    if (hasCodeDisplay) {
      ...
      if (emailButton) {
        ...
        console.log('이메일 전송 버튼을 찾을 수 없습니다. 페이지가 리다이렉트되었을 수 있습니다.');
    ...
    if (!emailButton) {
      ...
      // 인증코드 화면인지 확인
      let hasCodeDisplay = false;
      try {
        hasCodeDisplay = await page.$('#code-display');
      } catch (error) {
        console.log('인증코드 화면 확인 중 프레임 분리 에러:', error.message);
        console.log('페이지가 리다이렉트되었을 수 있습니다. 다음 단계로 진행합니다...');
      }
...
async function waitForAuthUrlFromGmail(options = {}) {
  const timeoutMs = options?.timeoutMs ?? 60000; // 원본과 동일하게 60초
  const pollMs = options?.pollIntervalMs ?? 1000; // 원본과 동일하게 1초
